...
func processInlineSecretsFromProfile(profilePath string, options *types.Options) (string, error) {
	...
	if err := os.MkdirAll(catalogconfig.DefaultSecretsTmpDir(), 0o700); err != nil {
		return "", fmt.Errorf("create inline secrets parent directory: %w", err)
	}
	tempDir, err := os.MkdirTemp(catalogconfig.DefaultSecretsTmpDir(), "nuclei-secrets-*")
	if err != nil {
