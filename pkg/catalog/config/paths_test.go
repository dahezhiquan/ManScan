...
func TestManScanDataPaths(t *testing.T) {
	if got, want := defaultConfigDir(), DefaultConfigDir(); got != want {
		t.Fatalf("default config directory = %q, want %q", got, want)
	}

	if got, want := (&Config{}).GetCacheDir(), DefaultCacheDir(); got != want {
		t.Fatalf("cache directory = %q, want %q", got, want)
	}

	if got, want := (&Config{}).GetStateDir(), DefaultConfigDir(); got != want {
		t.Fatalf("state directory = %q, want %q", got, want)
	}

	if got, want := DefaultTemplatesDir(), filepath.Join(DefaultDataRoot(), "templates"); got != want {
		t.Fatalf("templates directory = %q, want %q", got, want)
	}
}
