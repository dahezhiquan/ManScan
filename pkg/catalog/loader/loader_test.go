...
func TestLoadTemplatesWithTagsUsesConfigIgnoreFileInValidationMode(t *testing.T) {
	root := t.TempDir()
	...
	options.ExecutionId = "loader-tags-config-ignore"
...
`), 0o600)
	...
	require.Equal(t, initialUnverifiedCode, stats.GetValue(templates.SkippedUnverifiedCodeTemplateStats))
...
`), 0o600)
	...
	require.Len(t, loaded, 1)
	require.Equal(t, "unsigned-javascript-template", loaded[0].ID)
...
`), 0o600))
	...
	require.NoError(t, err)
	require.Len(t, loaded, 1)
	require.Equal(t, "cached-javascript", loaded[0].ID)

	refreshedMetadata, found := metadataIndex.Get(templatePath)
	require.True(t, found)
	require.True(t, refreshedMetadata.Verified)
...
`), 0o600)
	...
	require.Len(t, loaded, 1)
	require.Equal(t, "mixed-javascript-template", loaded[0].ID)
	require.Equal(t, initialUnverifiedJavascript, stats.GetValue(templates.SkippedUnverifiedJavascriptTemplateStats))
}
